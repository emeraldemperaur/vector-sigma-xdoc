export const xFormN8NSchemaSnippet = `import type { XFormType } from "@emeraldemperaur/vector-sigma";

export const xFormN8NSchema: XFormType = {
...
export const xFormAirflowDAGSchemaSnippet = `import type { XFormType } from "@emeraldemperaur/vector-sigma";

export const xFormDAGSchema: XFormType = {
...
export const xFormCodeN8NUsageSnippet = `import { VectorSigma } from '@emeraldemperaur/vector-sigma';
import { xFormN8NSchema } from './xFormN8NSchema';

export const App = () => {
  const xFormBuilder = new VectorSigma(xFormN8NSchema)
    .setName('N8N Reservation Booking')
    .setBrand("#EA4B71", "", 'right')
  ...
  return xFormBuilder.render({ 
    ...
    async onSubmit(values, actions, instance) {
      ...
      try {
        // Initiate HTTP POST request with 'values' and 'instance' payload
        await fetch(\`/api/restaurant/\${instance.formObject.uuid}/bookings\`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            formVersion: instance.formObject.uuid,
            responseTime: instance.timeSubmitted || Date.now() - instance.timeCreated,
        ...
        actions.resetForm(); 
        alert("Thank you for completing the VΣ booking!");
      } catch (error) {
...
export const xFormCodeAirflowUsageSnippet = `import { useVectorSigma } from '@emeraldemperaur/vector-sigma';
import { airflowOnboardingData } from './xFormDAGSchema';
